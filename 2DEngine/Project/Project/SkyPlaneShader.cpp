...
SkyPlaneShader::SkyPlaneShader(const SkyPlaneShader&)
{
}
...
bool SkyPlaneShader::Intilize(ID3D11Device* device, HWND hwnd)
{
	
	bool result;

	result = InitilizeShader(device, hwnd,  L"../Project/skyplane.vs", L"../Project/skyplane.ps");
	if(!result)
	{
		return false;
	}
...
void SkyPlaneShader::OutputErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool SkyPlaneShader:: InitilizeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	if(FAILED(result))
	{
		...
		if(errorMessage)
		{
			OutputErrorMessage(errorMessage, hwnd, vsFilename);
		}
	...
	if(FAILED(result))
	{
		...
		if(errorMessage)
		{
			OutputErrorMessage(errorMessage, hwnd, psFilename);
		}
