...
bool Map::Initialize(ID3D11Device* device, HWND hwnd, int screenWidth, int screenHeight, D3DXMATRIX viewMatrix
							  )
{
	bool result;
	...
	m_MiniMapBitmap = new HUDElement;
	if(!m_MiniMapBitmap)
	{
		return false;
	}
	...
	result = m_MiniMapBitmap->Initialize(device, screenWidth, screenHeight, L"../Project/data/colorm01.dds", 150, 150);
	if(!result)
	{
	...
	result = m_Border->Initialize(device, screenWidth, screenHeight, L"../Project/data/border01.dds", 154, 154);
	if(!result)
	{
	...
	result = m_Point->Initialize(device, screenWidth, screenHeight, L"../Project/data/View.png", 30, 30);
	if(!result)
	{
...
bool Map::Render(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX orthoMatrix, TextureShaderClass* textureShader)
{
	bool result;
...
void Map::PositionUpdate(float positionX, float positionZ)
{
